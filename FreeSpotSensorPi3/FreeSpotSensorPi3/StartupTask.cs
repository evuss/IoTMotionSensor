    ...
    public sealed class StartupTask : IBackgroundTask {
        ...
        private void GetSensorReadings() {
            ...
            try {
                ...
                if (pir == null) {  // Assume that initialization is needed
                    InitGPIO();
                }

                if (pir != null) {  // Only if we are intialized
                    ...
                    if(pirCurrentState != state.PendingStatus || state.PendingStatus == "") {  // A change. Start counting smoothing time. (Ie time with the same state required for change)
                    ...
                    if (state.PendingStatus != tmpMsg.Status &&     // This is a Status change AND....
                                ...
                                (state.PendingStatus == "F" && (DateTime.Now - state.PendingStatusTime).TotalSeconds >= settings.timerStateFSmoothing))) { // Pending status is "Free" and F-Smoothing-time has passed
                    ...
                    else if((DateTime.Now-state.LastSendTime).TotalSeconds > settings.timerKeepAliveIntervall) {  // No Change of status. Is it time fot "is alive"?
